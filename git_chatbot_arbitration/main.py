...
async def callback(request: Request):
    ...
    for event in events:
        if event["type"] == "message" and event["message"]["type"] == "text":
            ...
            user_name = source["userId"]

            # ユーザー発話を履歴に保存（グループ単位で）
            save_history(group_id, user_name, "user", user_msg)

            # ChatGPTで返答生成（user_nameごとに性格プロンプト分岐＋会話履歴参照）
...
# 会話履歴保存
def save_history(group_id, user_name, role, text):
    history_path = os.path.join(HISTORY_DIR, f"{group_id}.json")
    if os.path.exists(history_path):
        try:
            with open(history_path, "r", encoding="utf-8") as f:
...
# 性格データ推定（現在コメントアウト中）
def estimate_personality(group_id, user_name):
    ...
...
# GPT応答生成（会話履歴参照を追加）
def generate_gpt_reply(user_input, group_id, user_name):
    # 性格データ読み込み
    pers_path = os.path.join(PERSONALITY_DIR, f"{group_id}_{user_name}.json")
