...
def generate_gpt_reply(user_msg, group_id, user_name):
    ...
    messages = []
    #　シナリオ
    scenario = f"あなたと{user_name}は、昨日友人たちと食事会に参加しました。楽しかった雰囲気を共有したいと思い、以前{user_name}が「写真上げられるの苦手」と話していたことは頭にありながらも、その時{user_name}と2人で撮った写真を許可なく自身のブログに投稿しました。"

    # システムプロンプト
    personality_path = os.path.join(PERSONALITY_DIR, f"{user_name}.txt")
    system_prompt = f"あなたは「{bot_name}」です。{scenario} 過去の会話の流れを把握した上で、{user_name}の意見は少し聞きつつも、非を認める、歩み寄るといった行動はあまりせず、しっかり自分の主張をし続けてください。{user_name}との対立も辞さない態度でいてください。タメ口でしゃべってください。文章の長さは150〜300字程度にしてください。"
